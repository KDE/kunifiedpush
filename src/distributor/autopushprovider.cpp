...
void AutopushProvider::connectToProvider()
{
    ...
    connect(m_socket, &QWebSocket::pong, this, [](quint64 time, const QByteArray &msg) { qDebug() << time << msg; });
...
void AutopushProvider::wsMessageReceived(const QString &msg)
{
    ...
    if (msgType == "register"_L1) {
        ...
        Q_EMIT clientRegistered(m_currentClient);
        m_currentClient = {};
        return;
    }
    ...
    if (msgType == "unregister"_L1) {
        ...
        Q_EMIT clientUnregistered(m_currentClient);
        return;
    }
}
